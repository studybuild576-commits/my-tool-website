            ...
            <div className="relative group">
              ...
              <div className="absolute left-0 top-full mt-2 w-screen max-w-4xl bg-white shadow-2xl rounded-2xl border border-slate-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 p-8">
                      ...
                      <ul className="space-y-3">
                        {tools.slice(0, 5).map((tool) => (
                          <li key={tool.route}>
                            ...
                            >
                              <span className="text-xl group-hover/item:scale-110 transition">{tool.icon}</span>
                              <span>{tool.name}</span>
                            </Link>
                          </li>
                        ))}
                ...
                <div className="mt-6 pt-6 border-t border-slate-200">
                  <Link href="/tools" className="text-sm text-indigo-600 hover:text-purple-600 font-semibold inline-flex items-center gap-2">
